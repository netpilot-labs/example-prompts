...
> Run a vendor-migration rehearsal for a core refresh. Current state: two Cisco IOL core routers (CUR-CORE1, CUR-CORE2) sharing HSRP group 10 (virtual IP 10.1.10.1, CUR-CORE1 active with priority 110, preempt, and uplink interface tracking with decrement 20) on the user segment 10.1.10.0/24, OSPF area 0 between them, and eBGP from both cores to one shared upstream observer router for this side (FRR, AS 65100, loopback 192.0.2.1/32 advertised back to the cores over BGP), each core advertising 10.1.0.0/16 to it. Target state: the same design — same addressing, same tracking behavior — on two Arista cEOS devices (TGT-CORE1, TGT-CORE2) with VRRP group 10 replacing HSRP, as a fully separate topology with its own identically configured FRR observer (both target cores peered to it, same loopback advertised) so the two data planes never share a return path. Generate the full current-state IOS configs for this design first and show them to me — or, if I have pasted my sanitized production configs below, use those as the source instead and derive the device names, user segments, virtual IPs, and probe targets from my configs rather than the sample values, keeping the same validation steps. Then translate the Cisco configs to EOS, flagging every line that doesn't map one-to-one — HSRP-to-VRRP semantics, interface names, timer, preempt, and tracking defaults — and deploy both topologies in parallel, with a Linux host on each side's user segment. Diff-validate: routing tables and OSPF neighbor state must match between the current and target pairs, and the two observers must have received identical BGP advertisements from their respective sides. Then failover-test the target with two continuous pings running from its host — one to 10.1.10.1 and one to the observer loopback 192.0.2.1: fail TGT-CORE1, restore it, and confirm it is VRRP-active again with its OSPF and BGP sessions re-established before the next event; then drop TGT-CORE1's uplink (tracking should demote it) and restore it — report and grade seconds of loss per probe destination per event, both probes independently, and run the same two failures on the current pair as the baseline: the target passes an event by meeting or beating the current side's loss (equal counts as a pass). Finish with a go/no-go report: every check pass/fail with command output as evidence, plus the list of translated lines that need human review before cutover.
