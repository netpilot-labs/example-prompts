...
- A VSX failover battery — both node failures plus a counter-verified LAG-member drop — under dual probes from a host per VLAN per access switch (gateway + routed WAN loopback), each probe graded independently against explicit loss thresholds
